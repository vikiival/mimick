import {Abi, Bytes, encodeCall, decodeResult} from "@subsquid/ink-abi"

export const metadata = {
    "source": {
        "hash": "0xc657d73156ac962850748c3f208044d4af4b4d778ff5f5a6f9b90b6a3230f3d1",
        "language": "ink! 5.0.0",
        "compiler": "rustc 1.77.2",
        "build_info": {
            "build_mode": "Release",
            "cargo_contract_version": "4.1.1",
            "rust_toolchain": "stable-aarch64-apple-darwin",
            "wasm_opt_settings": {
                "keep_debug_symbols": false,
                "optimization_passes": "Z"
            }
        }
    },
    "contract": {
        "name": "erc721",
        "version": "5.0.0",
        "authors": [
            "Parity Technologies <admin@parity.io>"
        ]
    },
    "image": null,
    "spec": {
        "constructors": [
            {
                "args": [],
                "default": false,
                "docs": [
                    "Creates a new ERC-721 token contract."
                ],
                "label": "new",
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink_primitives",
                        "ConstructorResult"
                    ],
                    "type": 21
                },
                "selector": "0x9bae9d5e"
            }
        ],
        "docs": [],
        "environment": {
            "accountId": {
                "displayName": [
                    "AccountId"
                ],
                "type": 0
            },
            "balance": {
                "displayName": [
                    "Balance"
                ],
                "type": 36
            },
            "blockNumber": {
                "displayName": [
                    "BlockNumber"
                ],
                "type": 4
            },
            "chainExtension": {
                "displayName": [
                    "ChainExtension"
                ],
                "type": 39
            },
            "hash": {
                "displayName": [
                    "Hash"
                ],
                "type": 37
            },
            "maxEventTopics": 4,
            "staticBufferSize": 16384,
            "timestamp": {
                "displayName": [
                    "Timestamp"
                ],
                "type": 38
            }
        },
        "events": [
            {
                "args": [
                    {
                        "docs": [],
                        "indexed": true,
                        "label": "from",
                        "type": {
                            "displayName": [
                                "Option"
                            ],
                            "type": 25
                        }
                    },
                    {
                        "docs": [],
                        "indexed": true,
                        "label": "to",
                        "type": {
                            "displayName": [
                                "Option"
                            ],
                            "type": 25
                        }
                    },
                    {
                        "docs": [],
                        "indexed": true,
                        "label": "id",
                        "type": {
                            "displayName": [
                                "TokenId"
                            ],
                            "type": 4
                        }
                    }
                ],
                "docs": [
                    "Event emitted when a token transfer occurs."
                ],
                "label": "Transfer",
                "module_path": "erc721::erc721",
                "signature_topic": "0x01abfe3b4a94cbec268e6df12c117430904e0cd2d9cea73cfd1c023dfa83f46c"
            },
            {
                "args": [
                    {
                        "docs": [],
                        "indexed": true,
                        "label": "from",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    },
                    {
                        "docs": [],
                        "indexed": true,
                        "label": "to",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    },
                    {
                        "docs": [],
                        "indexed": true,
                        "label": "id",
                        "type": {
                            "displayName": [
                                "TokenId"
                            ],
                            "type": 4
                        }
                    }
                ],
                "docs": [
                    "Event emitted when a token approve occurs."
                ],
                "label": "Approval",
                "module_path": "erc721::erc721",
                "signature_topic": "0x8ab12ba91829833deb94db202a63546b5b65792283cef47282cd7f8ded9dcf3e"
            },
            {
                "args": [
                    {
                        "docs": [],
                        "indexed": true,
                        "label": "owner",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    },
                    {
                        "docs": [],
                        "indexed": true,
                        "label": "operator",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    },
                    {
                        "docs": [],
                        "indexed": false,
                        "label": "approved",
                        "type": {
                            "displayName": [
                                "bool"
                            ],
                            "type": 27
                        }
                    }
                ],
                "docs": [
                    "Event emitted when an operator is enabled or disabled for an owner.",
                    "The operator can manage all NFTs of the owner."
                ],
                "label": "ApprovalForAll",
                "module_path": "erc721::erc721",
                "signature_topic": "0xd774bd2aa72117f7287e75352336b69c11c6398b418daa34c89711939cfd45bb"
            }
        ],
        "lang_error": {
            "displayName": [
                "ink",
                "LangError"
            ],
            "type": 22
        },
        "messages": [
            {
                "args": [
                    {
                        "label": "owner",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    }
                ],
                "default": false,
                "docs": [
                    " Returns the balance of the owner.",
                    "",
                    " This represents the amount of unique tokens the owner has."
                ],
                "label": "balance_of",
                "mutates": false,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 23
                },
                "selector": "0x0f755a56"
            },
            {
                "args": [
                    {
                        "label": "id",
                        "type": {
                            "displayName": [
                                "TokenId"
                            ],
                            "type": 4
                        }
                    }
                ],
                "default": false,
                "docs": [
                    " Returns the owner of the token."
                ],
                "label": "owner_of",
                "mutates": false,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 24
                },
                "selector": "0x99720c1e"
            },
            {
                "args": [
                    {
                        "label": "id",
                        "type": {
                            "displayName": [
                                "TokenId"
                            ],
                            "type": 4
                        }
                    }
                ],
                "default": false,
                "docs": [
                    " Returns the approved account ID for this token if any."
                ],
                "label": "get_approved",
                "mutates": false,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 24
                },
                "selector": "0x27592dea"
            },
            {
                "args": [
                    {
                        "label": "owner",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    },
                    {
                        "label": "operator",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    }
                ],
                "default": false,
                "docs": [
                    " Returns `true` if the operator is approved by the owner."
                ],
                "label": "is_approved_for_all",
                "mutates": false,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 26
                },
                "selector": "0x0f5922e9"
            },
            {
                "args": [],
                "default": false,
                "docs": [],
                "label": "get_contract_uri",
                "mutates": false,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 28
                },
                "selector": "0x55cdacd6"
            },
            {
                "args": [
                    {
                        "label": "uri",
                        "type": {
                            "displayName": [
                                "Option"
                            ],
                            "type": 29
                        }
                    }
                ],
                "default": false,
                "docs": [],
                "label": "set_contract_uri",
                "mutates": true,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 30
                },
                "selector": "0x0b2fc649"
            },
            {
                "args": [
                    {
                        "label": "id",
                        "type": {
                            "displayName": [
                                "TokenId"
                            ],
                            "type": 4
                        }
                    }
                ],
                "default": false,
                "docs": [],
                "label": "token_uri",
                "mutates": false,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 33
                },
                "selector": "0x5b64e66a"
            },
            {
                "args": [
                    {
                        "label": "uri",
                        "type": {
                            "displayName": [
                                "Option"
                            ],
                            "type": 29
                        }
                    }
                ],
                "default": false,
                "docs": [],
                "label": "set_base_uri",
                "mutates": true,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 30
                },
                "selector": "0xec403dee"
            },
            {
                "args": [
                    {
                        "label": "to",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    },
                    {
                        "label": "approved",
                        "type": {
                            "displayName": [
                                "bool"
                            ],
                            "type": 27
                        }
                    }
                ],
                "default": false,
                "docs": [
                    " Approves or disapproves the operator for all tokens of the caller."
                ],
                "label": "set_approval_for_all",
                "mutates": true,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 30
                },
                "selector": "0xcfd0c27b"
            },
            {
                "args": [
                    {
                        "label": "to",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    },
                    {
                        "label": "id",
                        "type": {
                            "displayName": [
                                "TokenId"
                            ],
                            "type": 4
                        }
                    }
                ],
                "default": false,
                "docs": [
                    " Approves the account to transfer the specified token on behalf of the caller."
                ],
                "label": "approve",
                "mutates": true,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 30
                },
                "selector": "0x681266a0"
            },
            {
                "args": [
                    {
                        "label": "destination",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    },
                    {
                        "label": "id",
                        "type": {
                            "displayName": [
                                "TokenId"
                            ],
                            "type": 4
                        }
                    }
                ],
                "default": false,
                "docs": [
                    " Transfers the token from the caller to the given destination."
                ],
                "label": "transfer",
                "mutates": true,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 30
                },
                "selector": "0x84a15da1"
            },
            {
                "args": [
                    {
                        "label": "from",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    },
                    {
                        "label": "to",
                        "type": {
                            "displayName": [
                                "AccountId"
                            ],
                            "type": 0
                        }
                    },
                    {
                        "label": "id",
                        "type": {
                            "displayName": [
                                "TokenId"
                            ],
                            "type": 4
                        }
                    }
                ],
                "default": false,
                "docs": [
                    " Transfer approved or owned token."
                ],
                "label": "transfer_from",
                "mutates": true,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 30
                },
                "selector": "0x0b396f18"
            },
            {
                "args": [
                    {
                        "label": "id",
                        "type": {
                            "displayName": [
                                "TokenId"
                            ],
                            "type": 4
                        }
                    }
                ],
                "default": false,
                "docs": [
                    " Creates a new token."
                ],
                "label": "mint",
                "mutates": true,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 30
                },
                "selector": "0xcfdd9aa2"
            },
            {
                "args": [
                    {
                        "label": "id",
                        "type": {
                            "displayName": [
                                "TokenId"
                            ],
                            "type": 4
                        }
                    }
                ],
                "default": false,
                "docs": [
                    " Deletes an existing token. Only the owner can burn the token."
                ],
                "label": "burn",
                "mutates": true,
                "payable": false,
                "returnType": {
                    "displayName": [
                        "ink",
                        "MessageResult"
                    ],
                    "type": 30
                },
                "selector": "0xb1efc17b"
            }
        ]
    },
    "storage": {
        "root": {
            "layout": {
                "struct": {
                    "fields": [
                        {
                            "layout": {
                                "root": {
                                    "layout": {
                                        "leaf": {
                                            "key": "0x9edb16ed",
                                            "ty": 0
                                        }
                                    },
                                    "root_key": "0x9edb16ed",
                                    "ty": 3
                                }
                            },
                            "name": "token_owner"
                        },
                        {
                            "layout": {
                                "root": {
                                    "layout": {
                                        "leaf": {
                                            "key": "0x6076894d",
                                            "ty": 0
                                        }
                                    },
                                    "root_key": "0x6076894d",
                                    "ty": 9
                                }
                            },
                            "name": "token_approvals"
                        },
                        {
                            "layout": {
                                "root": {
                                    "layout": {
                                        "leaf": {
                                            "key": "0xf29d37b5",
                                            "ty": 4
                                        }
                                    },
                                    "root_key": "0xf29d37b5",
                                    "ty": 12
                                }
                            },
                            "name": "owned_tokens_count"
                        },
                        {
                            "layout": {
                                "root": {
                                    "layout": {
                                        "leaf": {
                                            "key": "0x334398ad",
                                            "ty": 8
                                        }
                                    },
                                    "root_key": "0x334398ad",
                                    "ty": 15
                                }
                            },
                            "name": "operator_approvals"
                        },
                        {
                            "layout": {
                                "leaf": {
                                    "key": "0x00000000",
                                    "ty": 19
                                }
                            },
                            "name": "contract_uri"
                        },
                        {
                            "layout": {
                                "leaf": {
                                    "key": "0x00000000",
                                    "ty": 19
                                }
                            },
                            "name": "base_uri"
                        }
                    ],
                    "name": "Erc721"
                }
            },
            "root_key": "0x00000000",
            "ty": 20
        }
    },
    "types": [
        {
            "id": 0,
            "type": {
                "def": {
                    "composite": {
                        "fields": [
                            {
                                "type": 1,
                                "typeName": "[u8; 32]"
                            }
                        ]
                    }
                },
                "path": [
                    "ink_primitives",
                    "types",
                    "AccountId"
                ]
            }
        },
        {
            "id": 1,
            "type": {
                "def": {
                    "array": {
                        "len": 32,
                        "type": 2
                    }
                }
            }
        },
        {
            "id": 2,
            "type": {
                "def": {
                    "primitive": "u8"
                }
            }
        },
        {
            "id": 3,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "K",
                        "type": 4
                    },
                    {
                        "name": "V",
                        "type": 0
                    },
                    {
                        "name": "KeyType",
                        "type": 5
                    }
                ],
                "path": [
                    "ink_storage",
                    "lazy",
                    "mapping",
                    "Mapping"
                ]
            }
        },
        {
            "id": 4,
            "type": {
                "def": {
                    "primitive": "u32"
                }
            }
        },
        {
            "id": 5,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "L",
                        "type": 6
                    },
                    {
                        "name": "R",
                        "type": 7
                    }
                ],
                "path": [
                    "ink_storage_traits",
                    "impls",
                    "ResolverKey"
                ]
            }
        },
        {
            "id": 6,
            "type": {
                "def": {
                    "composite": {}
                },
                "path": [
                    "ink_storage_traits",
                    "impls",
                    "AutoKey"
                ]
            }
        },
        {
            "id": 7,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "ParentKey",
                        "type": 8
                    }
                ],
                "path": [
                    "ink_storage_traits",
                    "impls",
                    "ManualKey"
                ]
            }
        },
        {
            "id": 8,
            "type": {
                "def": {
                    "tuple": []
                }
            }
        },
        {
            "id": 9,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "K",
                        "type": 4
                    },
                    {
                        "name": "V",
                        "type": 0
                    },
                    {
                        "name": "KeyType",
                        "type": 10
                    }
                ],
                "path": [
                    "ink_storage",
                    "lazy",
                    "mapping",
                    "Mapping"
                ]
            }
        },
        {
            "id": 10,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "L",
                        "type": 6
                    },
                    {
                        "name": "R",
                        "type": 11
                    }
                ],
                "path": [
                    "ink_storage_traits",
                    "impls",
                    "ResolverKey"
                ]
            }
        },
        {
            "id": 11,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "ParentKey",
                        "type": 8
                    }
                ],
                "path": [
                    "ink_storage_traits",
                    "impls",
                    "ManualKey"
                ]
            }
        },
        {
            "id": 12,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "K",
                        "type": 0
                    },
                    {
                        "name": "V",
                        "type": 4
                    },
                    {
                        "name": "KeyType",
                        "type": 13
                    }
                ],
                "path": [
                    "ink_storage",
                    "lazy",
                    "mapping",
                    "Mapping"
                ]
            }
        },
        {
            "id": 13,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "L",
                        "type": 6
                    },
                    {
                        "name": "R",
                        "type": 14
                    }
                ],
                "path": [
                    "ink_storage_traits",
                    "impls",
                    "ResolverKey"
                ]
            }
        },
        {
            "id": 14,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "ParentKey",
                        "type": 8
                    }
                ],
                "path": [
                    "ink_storage_traits",
                    "impls",
                    "ManualKey"
                ]
            }
        },
        {
            "id": 15,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "K",
                        "type": 16
                    },
                    {
                        "name": "V",
                        "type": 8
                    },
                    {
                        "name": "KeyType",
                        "type": 17
                    }
                ],
                "path": [
                    "ink_storage",
                    "lazy",
                    "mapping",
                    "Mapping"
                ]
            }
        },
        {
            "id": 16,
            "type": {
                "def": {
                    "tuple": [
                        0,
                        0
                    ]
                }
            }
        },
        {
            "id": 17,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "L",
                        "type": 6
                    },
                    {
                        "name": "R",
                        "type": 18
                    }
                ],
                "path": [
                    "ink_storage_traits",
                    "impls",
                    "ResolverKey"
                ]
            }
        },
        {
            "id": 18,
            "type": {
                "def": {
                    "composite": {}
                },
                "params": [
                    {
                        "name": "ParentKey",
                        "type": 8
                    }
                ],
                "path": [
                    "ink_storage_traits",
                    "impls",
                    "ManualKey"
                ]
            }
        },
        {
            "id": 19,
            "type": {
                "def": {
                    "primitive": "str"
                }
            }
        },
        {
            "id": 20,
            "type": {
                "def": {
                    "composite": {
                        "fields": [
                            {
                                "name": "token_owner",
                                "type": 3,
                                "typeName": "<Mapping<TokenId, AccountId> as::ink::storage::traits::\nAutoStorableHint<::ink::storage::traits::ManualKey<3977698206u32,\n()>,>>::Type"
                            },
                            {
                                "name": "token_approvals",
                                "type": 9,
                                "typeName": "<Mapping<TokenId, AccountId> as::ink::storage::traits::\nAutoStorableHint<::ink::storage::traits::ManualKey<1300854368u32,\n()>,>>::Type"
                            },
                            {
                                "name": "owned_tokens_count",
                                "type": 12,
                                "typeName": "<Mapping<AccountId, u32> as::ink::storage::traits::\nAutoStorableHint<::ink::storage::traits::ManualKey<3040321010u32,\n()>,>>::Type"
                            },
                            {
                                "name": "operator_approvals",
                                "type": 15,
                                "typeName": "<Mapping<(AccountId, AccountId), ()> as::ink::storage::traits::\nAutoStorableHint<::ink::storage::traits::ManualKey<2912437043u32,\n()>,>>::Type"
                            },
                            {
                                "name": "contract_uri",
                                "type": 19,
                                "typeName": "<String as::ink::storage::traits::AutoStorableHint<::ink::\nstorage::traits::ManualKey<436878892u32, ()>,>>::Type"
                            },
                            {
                                "name": "base_uri",
                                "type": 19,
                                "typeName": "<String as::ink::storage::traits::AutoStorableHint<::ink::\nstorage::traits::ManualKey<849877982u32, ()>,>>::Type"
                            }
                        ]
                    }
                },
                "path": [
                    "erc721",
                    "erc721",
                    "Erc721"
                ]
            }
        },
        {
            "id": 21,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "fields": [
                                    {
                                        "type": 8
                                    }
                                ],
                                "index": 0,
                                "name": "Ok"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 22
                                    }
                                ],
                                "index": 1,
                                "name": "Err"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 8
                    },
                    {
                        "name": "E",
                        "type": 22
                    }
                ],
                "path": [
                    "Result"
                ]
            }
        },
        {
            "id": 22,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "index": 1,
                                "name": "CouldNotReadInput"
                            }
                        ]
                    }
                },
                "path": [
                    "ink_primitives",
                    "LangError"
                ]
            }
        },
        {
            "id": 23,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "fields": [
                                    {
                                        "type": 4
                                    }
                                ],
                                "index": 0,
                                "name": "Ok"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 22
                                    }
                                ],
                                "index": 1,
                                "name": "Err"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 4
                    },
                    {
                        "name": "E",
                        "type": 22
                    }
                ],
                "path": [
                    "Result"
                ]
            }
        },
        {
            "id": 24,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "fields": [
                                    {
                                        "type": 25
                                    }
                                ],
                                "index": 0,
                                "name": "Ok"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 22
                                    }
                                ],
                                "index": 1,
                                "name": "Err"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 25
                    },
                    {
                        "name": "E",
                        "type": 22
                    }
                ],
                "path": [
                    "Result"
                ]
            }
        },
        {
            "id": 25,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "index": 0,
                                "name": "None"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 0
                                    }
                                ],
                                "index": 1,
                                "name": "Some"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 0
                    }
                ],
                "path": [
                    "Option"
                ]
            }
        },
        {
            "id": 26,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "fields": [
                                    {
                                        "type": 27
                                    }
                                ],
                                "index": 0,
                                "name": "Ok"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 22
                                    }
                                ],
                                "index": 1,
                                "name": "Err"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 27
                    },
                    {
                        "name": "E",
                        "type": 22
                    }
                ],
                "path": [
                    "Result"
                ]
            }
        },
        {
            "id": 27,
            "type": {
                "def": {
                    "primitive": "bool"
                }
            }
        },
        {
            "id": 28,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "fields": [
                                    {
                                        "type": 29
                                    }
                                ],
                                "index": 0,
                                "name": "Ok"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 22
                                    }
                                ],
                                "index": 1,
                                "name": "Err"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 29
                    },
                    {
                        "name": "E",
                        "type": 22
                    }
                ],
                "path": [
                    "Result"
                ]
            }
        },
        {
            "id": 29,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "index": 0,
                                "name": "None"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 19
                                    }
                                ],
                                "index": 1,
                                "name": "Some"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 19
                    }
                ],
                "path": [
                    "Option"
                ]
            }
        },
        {
            "id": 30,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "fields": [
                                    {
                                        "type": 31
                                    }
                                ],
                                "index": 0,
                                "name": "Ok"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 22
                                    }
                                ],
                                "index": 1,
                                "name": "Err"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 31
                    },
                    {
                        "name": "E",
                        "type": 22
                    }
                ],
                "path": [
                    "Result"
                ]
            }
        },
        {
            "id": 31,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "fields": [
                                    {
                                        "type": 8
                                    }
                                ],
                                "index": 0,
                                "name": "Ok"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 32
                                    }
                                ],
                                "index": 1,
                                "name": "Err"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 8
                    },
                    {
                        "name": "E",
                        "type": 32
                    }
                ],
                "path": [
                    "Result"
                ]
            }
        },
        {
            "id": 32,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "index": 0,
                                "name": "NotOwner"
                            },
                            {
                                "index": 1,
                                "name": "NotApproved"
                            },
                            {
                                "index": 2,
                                "name": "TokenExists"
                            },
                            {
                                "index": 3,
                                "name": "TokenNotFound"
                            },
                            {
                                "index": 4,
                                "name": "CannotInsert"
                            },
                            {
                                "index": 5,
                                "name": "CannotFetchValue"
                            },
                            {
                                "index": 6,
                                "name": "NotAllowed"
                            }
                        ]
                    }
                },
                "path": [
                    "erc721",
                    "erc721",
                    "Error"
                ]
            }
        },
        {
            "id": 33,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "fields": [
                                    {
                                        "type": 34
                                    }
                                ],
                                "index": 0,
                                "name": "Ok"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 22
                                    }
                                ],
                                "index": 1,
                                "name": "Err"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 34
                    },
                    {
                        "name": "E",
                        "type": 22
                    }
                ],
                "path": [
                    "Result"
                ]
            }
        },
        {
            "id": 34,
            "type": {
                "def": {
                    "variant": {
                        "variants": [
                            {
                                "index": 0,
                                "name": "None"
                            },
                            {
                                "fields": [
                                    {
                                        "type": 35
                                    }
                                ],
                                "index": 1,
                                "name": "Some"
                            }
                        ]
                    }
                },
                "params": [
                    {
                        "name": "T",
                        "type": 35
                    }
                ],
                "path": [
                    "Option"
                ]
            }
        },
        {
            "id": 35,
            "type": {
                "def": {
                    "tuple": [
                        19,
                        4
                    ]
                }
            }
        },
        {
            "id": 36,
            "type": {
                "def": {
                    "primitive": "u128"
                }
            }
        },
        {
            "id": 37,
            "type": {
                "def": {
                    "composite": {
                        "fields": [
                            {
                                "type": 1,
                                "typeName": "[u8; 32]"
                            }
                        ]
                    }
                },
                "path": [
                    "ink_primitives",
                    "types",
                    "Hash"
                ]
            }
        },
        {
            "id": 38,
            "type": {
                "def": {
                    "primitive": "u64"
                }
            }
        },
        {
            "id": 39,
            "type": {
                "def": {
                    "variant": {}
                },
                "path": [
                    "ink_env",
                    "types",
                    "NoChainExtension"
                ]
            }
        }
    ],
    "version": 5
}

const _abi = new Abi(metadata)

export function decodeEvent(bytes: Bytes, topics: Bytes[]): Event {
    return _abi.decodeEvent(bytes, topics)
}

export function decodeMessage(bytes: Bytes): Message {
    return _abi.decodeMessage(bytes)
}

export function decodeConstructor(bytes: Bytes): Constructor {
    return _abi.decodeConstructor(bytes)
}

export interface Chain {
    rpc: {
        call<T=any>(method: string, params?: unknown[]): Promise<T>
    }
}

export interface ChainContext {
    _chain: Chain
}

export class Contract {
    constructor(private ctx: ChainContext, private address: Bytes, private blockHash?: Bytes) { }

    balance_of(owner: AccountId): Promise<Result<TokenId, LangError>> {
        return this.stateCall('0x0f755a56', [owner])
    }

    owner_of(id: TokenId): Promise<Result<(AccountId | undefined), LangError>> {
        return this.stateCall('0x99720c1e', [id])
    }

    get_approved(id: TokenId): Promise<Result<(AccountId | undefined), LangError>> {
        return this.stateCall('0x27592dea', [id])
    }

    is_approved_for_all(owner: AccountId, operator: AccountId): Promise<Result<bool, LangError>> {
        return this.stateCall('0x0f5922e9', [owner, operator])
    }

    get_contract_uri(): Promise<Result<(string | undefined), LangError>> {
        return this.stateCall('0x55cdacd6', [])
    }

    token_uri(id: TokenId): Promise<Result<([string, TokenId] | undefined), LangError>> {
        return this.stateCall('0x5b64e66a', [id])
    }

    private async stateCall<T>(selector: string, args: any[]): Promise<T> {
        let input = _abi.encodeMessageInput(selector, args)
        let data = encodeCall(this.address, input)
        let result = await this.ctx._chain.rpc.call('state_call', ['ContractsApi_call', data, this.blockHash])
        let value = decodeResult(result)
        return _abi.decodeMessageOutput(selector, value)
    }
}

export type bool = boolean

export type LangError = LangError_CouldNotReadInput

export interface LangError_CouldNotReadInput {
    __kind: 'CouldNotReadInput'
}

export type TokenId = number

export type AccountId = Bytes

export type Constructor = Constructor_new

/**
 * Creates a new ERC-721 token contract.
 */
export interface Constructor_new {
    __kind: 'new'
}

export type Message = Message_approve | Message_balance_of | Message_burn | Message_get_approved | Message_get_contract_uri | Message_is_approved_for_all | Message_mint | Message_owner_of | Message_set_approval_for_all | Message_set_base_uri | Message_set_contract_uri | Message_token_uri | Message_transfer | Message_transfer_from

/**
 *  Approves the account to transfer the specified token on behalf of the caller.
 */
export interface Message_approve {
    __kind: 'approve'
    to: AccountId
    id: TokenId
}

/**
 *  Returns the balance of the owner.
 * 
 *  This represents the amount of unique tokens the owner has.
 */
export interface Message_balance_of {
    __kind: 'balance_of'
    owner: AccountId
}

/**
 *  Deletes an existing token. Only the owner can burn the token.
 */
export interface Message_burn {
    __kind: 'burn'
    id: TokenId
}

/**
 *  Returns the approved account ID for this token if any.
 */
export interface Message_get_approved {
    __kind: 'get_approved'
    id: TokenId
}

export interface Message_get_contract_uri {
    __kind: 'get_contract_uri'
}

/**
 *  Returns `true` if the operator is approved by the owner.
 */
export interface Message_is_approved_for_all {
    __kind: 'is_approved_for_all'
    owner: AccountId
    operator: AccountId
}

/**
 *  Creates a new token.
 */
export interface Message_mint {
    __kind: 'mint'
    id: TokenId
}

/**
 *  Returns the owner of the token.
 */
export interface Message_owner_of {
    __kind: 'owner_of'
    id: TokenId
}

/**
 *  Approves or disapproves the operator for all tokens of the caller.
 */
export interface Message_set_approval_for_all {
    __kind: 'set_approval_for_all'
    to: AccountId
    approved: bool
}

export interface Message_set_base_uri {
    __kind: 'set_base_uri'
    uri?: (string | undefined)
}

export interface Message_set_contract_uri {
    __kind: 'set_contract_uri'
    uri?: (string | undefined)
}

export interface Message_token_uri {
    __kind: 'token_uri'
    id: TokenId
}

/**
 *  Transfers the token from the caller to the given destination.
 */
export interface Message_transfer {
    __kind: 'transfer'
    destination: AccountId
    id: TokenId
}

/**
 *  Transfer approved or owned token.
 */
export interface Message_transfer_from {
    __kind: 'transfer_from'
    from: AccountId
    to: AccountId
    id: TokenId
}

export type Event = Event_Approval | Event_ApprovalForAll | Event_Transfer

export interface Event_Approval {
    __kind: 'Approval'
    from: AccountId
    to: AccountId
    id: TokenId
}

export interface Event_ApprovalForAll {
    __kind: 'ApprovalForAll'
    owner: AccountId
    operator: AccountId
    approved: bool
}

export interface Event_Transfer {
    __kind: 'Transfer'
    from?: (AccountId | undefined)
    to?: (AccountId | undefined)
    id: TokenId
}

export type Result<T, E> = {__kind: 'Ok', value: T} | {__kind: 'Err', value: E}
